...
class Advent7Test : FreeSpec({
    "does containers with the input" {
        val input = """
        ...
        advent7a(input) shouldBe 4
    }

    "expresses a sentence in a Bag object" - {
        "with no bags" {
            Advent7().bag(
                "faded blue bags contain no other bags."
            ) shouldBe Bag(
        ...
        "with one kind of bag" {
            Advent7().bag(
                "bright white bags contain 1 shiny gold bag."
            ) shouldBe Bag(
                colour = "bright white",
                containing = listOf(BagRule(1, "shiny gold"))
            )
        }
        "with multiple kinds of bags" {
            Advent7().bag(
                "vibrant plum bags contain 5 faded blue bags, 6 dotted black bags."
            ) shouldBe Bag(
                colour = "vibrant plum",
                containing = listOf(
                    BagRule(5, "faded blue"),
                    BagRule(6, "dotted black")
                ),
            )
        }
    }

    "finds all possible direct containers for a bag" - {
        val bags = listOf(
            Bag(colour = "yellow", containing = listOf(BagRule(1, "blue"))),
            Bag(colour = "red", containing = listOf(BagRule(1, "blue"))),
            Bag(colour = "green", containing = listOf(BagRule(1, "red"))),
            Bag(colour = "blue")
        )

        "if there's one" {
            Advent7().directContainers("red", bags) shouldBe listOf(
                Bag(colour = "green", containing = listOf(BagRule(1, "red")))
            )
        }

        "if there are none" {
            Advent7().directContainers("green", bags) shouldBe emptyList()
        }

        "if there are multiple" {
            Advent7().directContainers("blue", bags) shouldBe listOf(
                Bag(colour = "yellow", containing = listOf(BagRule(1, "blue"))),
                Bag(colour = "red", containing = listOf(BagRule(1, "blue")))
            )
        }
    ...
    "finds all possible containers" - {
        ...
        "if there are some one level up" {
            Advent7().allContainers("red", bags) shouldBe listOf(
                Bag(colour = "green", containing = listOf(BagRule(1, "red")))
            )
        }

        "if there are none" {
            Advent7().allContainers("green", bags) shouldBe emptyList()
        }

        "if there are multiple levels" {
            Advent7().allContainers("blue", bags) shouldContainExactlyInAnyOrder  listOf(
                Bag(colour = "yellow", containing = listOf(BagRule(1, "blue"))),
                Bag(colour = "red", containing = listOf(BagRule(1, "blue"))),
                Bag(colour = "green", containing = listOf(BagRule(1, "red")))
            )
    ...
    "does containers with the puzzle input" {
        advent7a(puzzle7) shouldBe 261
    }

    "counts the number of bags directly inside another" - {
        "with no bags" {
            Advent7().bagsDirectlyInside("red", listOf(
                Bag("red")
            )) shouldBe emptyList()
        }
        "with some bags" {
            Advent7().bagsDirectlyInside("red", listOf(
                Bag("red", listOf(BagRule(2, "blue")))
            )) shouldBe listOf("blue", "blue")
        }
    }

    "counts all of the bags inside another" - {
        "with one level of bags" {
            Advent7().bagsInside("red", listOf(
                Bag("red", listOf(BagRule(2, "blue")))
            )) shouldContainExactlyInAnyOrder listOf("red", "blue", "blue")
        }
        "with more tiers of bags" {
            Advent7().bagsInside("red", listOf(
                Bag("red", listOf(BagRule(2, "blue"))),
                Bag("blue", listOf(BagRule(3, "yellow")))
    ...
    "counts bags for the sample input" {
        advent7b("""
        ...
        """.trimIndent()) shouldBe 126
    }

    "counts bags for puzzle input" {
        advent7b(puzzle7) shouldBe 3765
    }
})
